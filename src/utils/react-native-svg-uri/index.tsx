...
function fixYPosition(y: any, node: any): string {
  if (node.attributes) {
...
interface SvgUriState {
  fill?: string;
  svgXmlData?: string;
}

export default class SvgUri extends React.Component<SvgUriProps, SvgUriState> {
  private isComponentMounted: boolean;
  constructor(props: Readonly<SvgUriProps>) {
    super(props);
  ...
  componentWillReceiveProps(nextProps: SvgUriProps) {
    if (nextProps.source) {
  ...
  async fetchSVGData(uri: any) {
    ...
    try {
      ...
      if (this.isComponentMounted) {
        this.setState({ svgXmlData: responseXML as string }, () => {
  ...
  trimElementChilden(children: any) {
  ...
  obtainComponentAtts({ attributes }: any, enabledAttributes: string[]) {
    const styleAtts: any = {};
    ...
    const componentAtts = Array.from(attributes)
      ...
      .reduce((acc: any, { nodeName, nodeValue }: { nodeName: string; nodeValue: any }) => {
  ...
  render() {
    try {
      ...
      const doc: any = new xmldom.DOMParser().parseFromString(inputSVG);
